...
pub fn Button(
    ...
    #[prop(optional)] class: ButtonClassSlots,
    #[prop(optional, into)] selected: Signal<bool>,
    children: Children,
) -> impl IntoView {
    let default_outer_class =
        "p-1 bg-green text-white rounded-sm group cursor-pointer transition-colors duration-300";
    ...
    view! {
        ...
        >
            <div
                class=inner_class
                class=(["bg-green-100", "outline", "outline-offset-[-1px]"], selected)
            >
                {children()}
            </div>
        </button>
